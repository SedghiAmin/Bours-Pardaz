...
import 'package:bourse_pardaz/class/MoneyItems.dart';
...
Future<List<MoneyItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/PowerfullSeller2.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<MoneyItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<MoneyItems>((json) => MoneyItems.fromJson(json)).toList();
}

class PowerfullSellerResult extends StatefulWidget {
  final String title;
...
/*   final FlutterSecureStorage localStorageMobile;
  ...
  @override
  _PowerfullSellerResultState createState() => _PowerfullSellerResultState();
}

class _PowerfullSellerResultState extends State<PowerfullSellerResult> {
              ...
              child: FutureBuilder(
                ...
                builder: (context, snapshot) {
                  ...
                  return snapshot.hasData
                      ? GenerateCard(items: snapshot.data as List<MoneyItems>)
                      : Column(
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      ...
      widget.items.sort((row1, row2) => compareDouble(
          ascending, double.parse(row1.avgS), double.parse(row2.avgS)));
    } else if (columnIndex == 2) {
      widget.items.sort((row1, row2) => compareDouble(
          ascending,
          double.parse(row1.avgS) / double.parse(row1.avgB),
          double.parse(row2.avgS) / double.parse(row2.avgB)));
