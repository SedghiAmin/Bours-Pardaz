...
Future<List<SafItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/SafeKharid2.json';
...
class SafeKharidResult extends StatefulWidget {
  final String title;
  ...
  @override
  _SafeKharidResultState createState() => _SafeKharidResultState();
}

class _SafeKharidResultState extends State<SafeKharidResult> {
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      ...
      widget.items.sort((row1, row2) => compareDouble(
          ascending,
          double.parse((row1.dvol).replaceAll(RegExp(r"[^\s\w]"), '')),
          double.parse((row2.dvol).replaceAll(RegExp(r"[^\s\w]"), ''))));
