...
import 'package:bourse_pardaz/class/NavasanItems.dart';
...
Future<List<NavasanItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/FinishPositive2.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<NavasanItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed
      .map<NavasanItems>((json) => NavasanItems.fromJson(json))
      .toList();
}

class FinishPositiveResult extends StatefulWidget {
  final String title;
  ...
  @override
  _FinishPositiveResultState createState() => _FinishPositiveResultState();
}

class _FinishPositiveResultState extends State<FinishPositiveResult> {
              ...
              child: FutureBuilder(
                ...
                builder: (context, snapshot) {
                  ...
                  return snapshot.hasData
                      ? GenerateCard(items: snapshot.data as List<NavasanItems>)
                      : Column(
...
class _GenerateCardState extends State<GenerateCard> {
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      ...
      widget.items
          .sort((row1, row2) => compareString(ascending, row1.pcp, row2.pcp));
