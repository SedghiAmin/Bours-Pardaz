...
import 'package:bourse_pardaz/class/TickItems.dart';
import 'package:persian_number_utility/persian_number_utility.dart';
//import 'package:shared_preferences/shared_preferences.dart';

Future<List<TickItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/Tick2.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<TickItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<TickItems>((json) => TickItems.fromJson(json)).toList();
}

class TickResult extends StatefulWidget {
  final String title;
  ...
  @override
  _TickResultState createState() => _TickResultState();
}

class _TickResultState extends State<TickResult> {
              ...
              child: FutureBuilder(
                ...
                builder: (context, snapshot) {
                  ...
                  return snapshot.hasData
                      ? GenerateCard(items: snapshot.data as List<TickItems>)
                      : Column(
