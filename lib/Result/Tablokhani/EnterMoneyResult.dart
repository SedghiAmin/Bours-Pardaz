...
Future<List<MoneyItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/EnterMoney2.json';
...
class EnterMoneyResult extends StatefulWidget {
  final String title;
  ...
  @override
  _EnterMoneyResultState createState() => _EnterMoneyResultState();
}

class _EnterMoneyResultState extends State<EnterMoneyResult> {
                                    ...
                                    DataCell(Container(
                                      ...
                                      child: Text(
                                        NumberUtility.seRagham(
                                          NumberUtility.changeDigit(
                                              (int.parse(element.avgB) *
                                                      int.parse((element.pc)
                                                          .replaceAll(
                                                              RegExp(
                                                                  r"[^\s\w]"),
                                                              '')))
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      ...
      widget.items.sort((row1, row2) => compareDouble(
          ascending,
          (double.parse(row1.avgB) *
              double.parse((row1.pc).replaceAll(RegExp(r"[^\s\w]"), ''))),
          (double.parse(row2.avgB) *
              double.parse((row2.pc).replaceAll(RegExp(r"[^\s\w]"), '')))));
