...
import 'package:bourse_pardaz/class/TrendUpItems.dart';
...
Future<List<TrendUpItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/TrendUp.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<TrendUpItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed
      .map<TrendUpItems>((json) => TrendUpItems.fromJson(json))
      .toList();
}

class TrendeUpResult extends StatefulWidget {
  final String title;
  ...
  @override
  _TrendeUpResultState createState() => _TrendeUpResultState();
}

class _TrendeUpResultState extends State<TrendeUpResult> {
              ...
              child: FutureBuilder(
                ...
                builder: (context, snapshot) {
                  ...
                  return snapshot.hasData
                      ? GenerateCard(items: snapshot.data as List<TrendUpItems>)
                      : Column(
