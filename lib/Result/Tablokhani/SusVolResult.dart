...
import 'package:bourse_pardaz/class/SusVolItems.dart';
import 'package:persian_number_utility/persian_number_utility.dart';
//import 'package:shared_preferences/shared_preferences.dart';

Future<List<SusVolItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/tablo/SusVol.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<SusVolItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<SusVolItems>((json) => SusVolItems.fromJson(json)).toList();
}

class SusVolResult extends StatefulWidget {
  final String title;
 ...
 /*  final FlutterSecureStorage localStorageMobile;
  ...
  @override
  _SusVolResultState createState() => _SusVolResultState();
}

class _SusVolResultState extends State<SusVolResult> {
              ...
              child: FutureBuilder(
                ...
                builder: (context, snapshot) {
                  ...
                  return snapshot.hasData
                      ? GenerateCard(items: snapshot.data as List<SusVolItems>)
                      : Column(
...
class _GenerateCardState extends State<GenerateCard> {
  ...
  void onSort(int columnIndex, bool ascending) {
    if (columnIndex == 0) {
      ...
      widget.items.sort((row1, row2) => compareDouble(
          ascending,
          double.parse((row1.volW).replaceAll(RegExp(r"[^\s\w]"), '')),
          double.parse((row2.volW).replaceAll(RegExp(r"[^\s\w]"), ''))));
    }
