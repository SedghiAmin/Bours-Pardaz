...
Future<List<HammerItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/daily/threeWhiteSoldiers.json';
...
Future<List<HammerItems>> fetchWeeklyItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/weekly/threeWhiteSoldiers.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}
...
Future<List<HammerItems>> fetchMonthlyItems(Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/priceAction/monthly/threeWhiteSoldiers.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}
...
class ThreeWhiteSoldiersResult extends StatefulWidget {
  final String title;
...
/*       required this.localStorageMobile,
  ...
  @override
  _ThreeWhiteSoldiersResultState createState() =>
      _ThreeWhiteSoldiersResultState();
}

class _ThreeWhiteSoldiersResultState extends State<ThreeWhiteSoldiersResult> {
                        ...
                        onPressed: () => Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => DescriptionActionPrice(
                              ...
                              patternTitle: 'سه سرباز سفید‌پوش',
