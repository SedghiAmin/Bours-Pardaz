...
Future<List<HammerItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/daily/invertedHammer.json';
...
Future<List<HammerItems>> fetchWeeklyItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/weekly/invertedHammer.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}
...
Future<List<HammerItems>> fetchMonthlyItems(Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/priceAction/monthly/invertedHammer.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}
...
class InvertedHammerResult extends StatefulWidget {
  final String title;
  ...
  @override
  _InvertedHammerResultState createState() => _InvertedHammerResultState();
}

class _InvertedHammerResultState extends State<InvertedHammerResult> {
