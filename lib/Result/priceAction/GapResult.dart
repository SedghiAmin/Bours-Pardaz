...
import 'package:bourse_pardaz/class/GapItems.dart';
import 'package:persian_number_utility/persian_number_utility.dart';
//import 'package:shared_preferences/shared_preferences.dart';

Future<List<GapItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/daily/gap.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<GapItems>> fetchWeeklyItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/weekly/gap.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<GapItems>> fetchMonthlyItems(Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/priceAction/monthly/gap.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<GapItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<GapItems>((json) => GapItems.fromJson(json)).toList();
}

class GapResult extends StatefulWidget {
  final String title;
  ...
  @override
  _GapResultState createState() => _GapResultState();
}

class _GapResultState extends State<GapResult> {
                      ...
                      children: [
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].low1, NumStrLanguage.Farsi),
                          ),
                          textAlign: TextAlign.center,
                        ),
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].high1, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                    ...
                    child: Column(
                      children: [
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].low2, NumStrLanguage.Farsi),
                          ),
                          textAlign: TextAlign.center,
                        ),
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].high2, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            backgroundColor: Color(0xffeaddd4),
                          ),
