...
Future<List<HammerItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/daily/morningStar.json';
...
Future<List<HammerItems>> fetchWeeklyItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/priceAction/weekly/morningStar.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}
...
Future<List<HammerItems>> fetchMonthlyItems(Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/priceAction/monthly/morningStar.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}
...
class MorningStarResult extends StatefulWidget {
  final String title;
  ...
  @override
  _MorningStarResultState createState() => _MorningStarResultState();
}

class _MorningStarResultState extends State<MorningStarResult> {
                        ...
                        onPressed: () => Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => DescriptionActionPrice(
                              ...
                              patternTitle: 'الگوی ستاره صبحگاهی',
