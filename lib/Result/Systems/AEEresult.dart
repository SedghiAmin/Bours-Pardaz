...
import 'package:bourse_pardaz/class/AEEitems.dart';
...
Future<List<AEEitems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/strategy/AEE.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<AEEitems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<AEEitems>((json) => AEEitems.fromJson(json)).toList();
}

class AEEresult extends StatefulWidget {
  final String title;
  ...
  @override
  _AEEresultState createState() => _AEEresultState();
}

class _AEEresultState extends State<AEEresult> {
                      ...
                      onPressed: () => Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => DescriptionSystems(
                            ...
                            description: '',
                            filter: 'AEE',
                            filterTitle: 'ADX(15) + EMA(5) + EMA(10)',
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return snapshot.hasData
                              ? GenerateCard(
                                  items: snapshot.data as List<AEEitems>)
                              : Column(
                    ...
                    child: Column(
                      children: [
                        ...
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].ema9, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            fontSize: 10,
                            backgroundColor: Color(0xffeaddd4),
