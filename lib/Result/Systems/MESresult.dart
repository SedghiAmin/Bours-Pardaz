...
import 'package:bourse_pardaz/class/MESitems.dart';
...
Future<List<MESitems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/strategy/MES.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<MESitems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<MESitems>((json) => MESitems.fromJson(json)).toList();
}

class MESresult extends StatefulWidget {
  final String title;
  ...
  @override
  _MESresultState createState() => _MESresultState();
}

class _MESresultState extends State<MESresult> {
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return snapshot.hasData
                              ? GenerateCard(
                                  items: snapshot.data as List<MESitems>)
                              : Column(
                      ...
                      children: [
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].ema, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            fontSize: 10,
                          ),
