...
import 'package:bourse_pardaz/class/SRSitems.dart';
...
Future<List<SRSitems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/strategy/SRS.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<SRSitems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<SRSitems>((json) => SRSitems.fromJson(json)).toList();
}

class SRSresult extends StatefulWidget {
  final String title;
  ...
  @override
  _SRSresultState createState() => _SRSresultState();
}

class _SRSresultState extends State<SRSresult> {
                      ...
                      onPressed: () => Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => DescriptionSystems(
                            ...
                            filter: 'SRS',
                            filterTitle:
                                'Stochastic(5,3,3) + RSI(14) + SMA(144)',
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return snapshot.hasData
                              ? GenerateCard(
                                  items: snapshot.data as List<SRSitems>)
                              : Column(
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].sma, NumStrLanguage.Farsi),
                      ),
                      style: TextStyle(
                        fontSize: 10,
                      ),
