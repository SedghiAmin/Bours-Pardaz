...
import 'package:bourse_pardaz/class/BRitems.dart';
...
Future<List<BRitems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/strategy/BR.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<BRitems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<BRitems>((json) => BRitems.fromJson(json)).toList();
}

class BRresult extends StatefulWidget {
  final String title;
  ...
  @override
  _BRresultState createState() => _BRresultState();
}

class _BRresultState extends State<BRresult> {
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return snapshot.hasData
                              ? GenerateCard(
                                  items: snapshot.data as List<BRitems>)
                              : Column(
                    ...
                    child: Column(
                      children: [
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].closePrice, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            fontSize: 10,
                          ),
                        ...
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].stopLost, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            fontSize: 10,
                            backgroundColor: Color(0xffeaddd4),
