...
import 'package:bourse_pardaz/class/RSEEitems.dart';
...
Future<List<RSEEitems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/strategy/RSEE.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<RSEEitems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<RSEEitems>((json) => RSEEitems.fromJson(json)).toList();
}

class RSEEresult extends StatefulWidget {
  final String title;
...
/*       required this.localStorageMobile,
  ...
  @override
  _RSEEresultState createState() => _RSEEresultState();
}

class _RSEEresultState extends State<RSEEresult> {
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return snapshot.hasData
                              ? GenerateCard(
                                  items: snapshot.data as List<RSEEitems>)
                              : Column(
              ...
              child: new Container(
                ...
                child: new Card(
                  ...
                  child: Padding(
                    ...
                    child: Center(
                      child: Text(
                        'RSI(14) + Stochastic(14,3,3) + EMA(3) + EMA(8)',
                        style: TextStyle(
                          color: Color(0xff318d90),
                          fontWeight: FontWeight.bold,
                          fontSize: 12,
                      ...
                      children: [
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].ema3, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            fontSize: 10,
                          ),
                        ...
                        Text(
                          NumberUtility.seRagham(
                            NumberUtility.changeDigit(
                                items[index].ema6, NumStrLanguage.Farsi),
                          ),
                          style: TextStyle(
                            fontSize: 10,
                            backgroundColor: Color(0xffeaddd4),
