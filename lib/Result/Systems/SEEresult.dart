...
import 'package:bourse_pardaz/class/SEEitems.dart';
...
Future<List<SEEitems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/strategy/SEE.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<SEEitems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<SEEitems>((json) => SEEitems.fromJson(json)).toList();
}
                      ...
                      child: FutureBuilder(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return snapshot.hasData
                              ? GenerateCard(
                                  items: snapshot.data as List<SEEitems>)
                              : Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].enter, NumStrLanguage.Farsi),
                      ),
                      style: TextStyle(
                        fontSize: 10,
                      ),
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].target, NumStrLanguage.Farsi),
                      ),
                      style: TextStyle(
                        fontSize: 10,
                      ),
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].stop, NumStrLanguage.Farsi),
                      ),
                      style: TextStyle(
                        fontSize: 10,
                      ),
