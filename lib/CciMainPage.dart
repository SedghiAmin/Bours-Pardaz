...
import 'package:bourse_pardaz/class/CciItems.dart';
import 'package:persian_number_utility/persian_number_utility.dart';
//import 'package:shared_preferences/shared_preferences.dart';

Future<List<CciItems>> fetchItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/daily/cci.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<CciItems>> fetchWeeklyItems(Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/weekly/cci.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<CciItems>> fetchMonthlyItems(Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/indicators/monthly/cci.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<CciItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<CciItems>((json) => CciItems.fromJson(json)).toList();
}

class CciMainPage extends StatefulWidget {
  final String title;
  ...
  @override
  _CciMainPageState createState() => _CciMainPageState();
}

class _CciMainPageState extends State<CciMainPage> {
                              ...
                              child: Column(
                                children: [
                                  Padding(
                                    padding: const EdgeInsets.only(top: 30.0),
                                    child: Text(
                                      'CCI(5) اشاره به تایم فریم 5 هفته اخیر دارد.',
                                      style: TextStyle(
                                        fontSize: 11,
                                      ),
                                    ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].cciFast, NumStrLanguage.Farsi),
                      ),
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].cci, NumStrLanguage.Farsi),
                      ),
                      textDirection: TextDirection.ltr,
                      textAlign: TextAlign.center,
                    ),
