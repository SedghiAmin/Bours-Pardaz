...
import 'package:bourse_pardaz/class/MacdItems.dart';
...
Future<List<MacdItems>> fetchItems(http.Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/daily/macd.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<MacdItems>> fetchWeeklyItems(http.Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/weekly/macd.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<MacdItems>> fetchMonthlyItems(http.Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/indicators/monthly/macd.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<MacdItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<MacdItems>((json) => MacdItems.fromJson(json)).toList();
}
...
class MacdMainPage extends StatefulWidget {
  final String title;
  ...
  @override
  _MacdMainPageState createState() => _MacdMainPageState();
}

class _MacdMainPageState extends State<MacdMainPage> {
