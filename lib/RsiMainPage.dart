...
import 'package:bourse_pardaz/class/RsiItems.dart';
import 'package:persian_number_utility/persian_number_utility.dart';
//import 'package:shared_preferences/shared_preferences.dart';

Future<List<RsiItems>> fetchItems(http.Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/daily/rsi.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<RsiItems>> fetchWeeklyItems(http.Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/weekly/rsi.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<RsiItems>> fetchMonthlyItems(http.Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/indicators/monthly/rsi.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<RsiItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed.map<RsiItems>((json) => RsiItems.fromJson(json)).toList();
}
...
class RsiMainPage extends StatefulWidget {
  final String title;
  ...
  @override
  _RsiMainPageState createState() => _RsiMainPageState();
}

class _RsiMainPageState extends State<RsiMainPage> {
