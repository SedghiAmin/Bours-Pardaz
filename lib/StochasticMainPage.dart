...
import 'package:bourse_pardaz/class/StochasticItems.dart';
import 'package:persian_number_utility/persian_number_utility.dart';
//import 'package:shared_preferences/shared_preferences.dart';

Future<List<StochasticItems>> fetchItems(http.Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/daily/stochastic.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<StochasticItems>> fetchWeeklyItems(http.Client client) async {
  String url =
      'https://s3.ir-thr-at1.arvanstorage.com/boursepardaz/vip/1/indicators/weekly/stochastic.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

Future<List<StochasticItems>> fetchMonthlyItems(http.Client client) async {
  String url =
      'https://bourspardaz.s3.ir-tbz-sh1.arvanstorage.com/indicators/monthly/stochastic.json';
  final response = await client.get(Uri.parse(url));
  //return parseItems(response);
  return compute(parseItems, utf8.decode(response.bodyBytes));
}

List<StochasticItems> parseItems(String response) {
  final parsed = json.decode(response).cast<Map<String, dynamic>>();

  return parsed
      .map<StochasticItems>((json) => StochasticItems.fromJson(json))
      .toList();
}
...
class StochasticMainPage extends StatefulWidget {
  final String title;
  ...
  @override
  _StochasticMainPageState createState() => _StochasticMainPageState();
}

class _StochasticMainPageState extends State<StochasticMainPage> {
...
class GenerateCard extends StatelessWidget {
  final List<StochasticItems> items;
                  ...
                  Container(
                    ...
                    child: Text(
                      NumberUtility.seRagham(
                        NumberUtility.changeDigit(
                            items[index].main, NumStrLanguage.Farsi),
                      ),
